...
\subsubsection*{Experiment 2}
 ...
 This assumption derives from the concept that implicit water models can only assume an NVT ensemble, because pressure has no physical meaning deal to the absence of solvent. It is supposed that the entropy that comes from solvent interaction    just by varying the size of simulation box.  
 
 The experiment begins with the development of two new coarse-grain models based on M1 from the previous experiment. However, at this moment the all-atoms box's concentrations were increased to $C_{B2} = 1.62\ mM$ and $C_{B3} = 3.30\ mM$, thus the Intermediate Concentration Model ($M_{I}$) and High Concentration Model ($M_{H}$) were created based on those concentrations respectively.
